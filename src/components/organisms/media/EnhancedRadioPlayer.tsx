...
import { Play, Pause, VolumeX, Volume1, Volume2, History, SkipForward, Users, Radio } from 'lucide-react'
...
export function EnhancedRadioPlayer({
  ...
  const [activeTab, setActiveTab] = useState<'player' | 'history' | 'next'>('player')
  ...
  return (
    <div className={cn("w-full max-w-md mx-auto bg-card border rounded-xl shadow-lg overflow-hidden", className)}>
      ...
      <div className="p-6">
                ...
                <div className="absolute inset-0 flex items-center">
                  {[...Array(5)].map((_, i) => (
                    <div
                      key={i}
                      className={cn(
                        "w-0.5 h-3 rounded-full transition-colors duration-200",
                        volume > (i + 1) * 20 ? "bg-primary/60" : "bg-muted"
                      )}
                      style={{ left: `${(i + 1) * 20}%` }}
                    />
                  ))}
        ...
        {activeTab === 'next' && showNextPlaying && (
          <NextPlayingCard playingNext={playingNext} />
        )}

        {activeTab === 'history' && showSongHistory && (
